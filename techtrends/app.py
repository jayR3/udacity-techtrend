...
def fetch_articles_count(metrics_data):
   
    connection = get_db_connection()
    article_count = connection.execute('SELECT count(*) FROM posts').fetchone()
    connection.close()

    metrics_data['db_connection_count'] += 1
    metrics_data['post_count'] = article_count[0]





#confirm if there is post page
...
def metrics():
    ...
    fetch_articles_count(metrics_data)
